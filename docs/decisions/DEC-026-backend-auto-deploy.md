...
- 결정일: 2026-08-10
- 참여자: **[팀 확인 필요]**
- 관련 문서·Issue: `docs/deployment.md`, DEC-014, DEC-023, DEC-025, **[Issue 번호 필요]**
- 대체하는 결정: DEC-025의 "첫 개발에서는 자동 배포를 만들지 않는다" 항목

## 결정할 문제

DEC-025와 `mvp-scope.md`는 첫 개발에서 자동 배포를 만들지 않고 DEV EC2에 수동 배포한다고 정했다.
수동 배포를 반복하면서 배포 담당자 1명에게 절차가 묶이고, 배포 시점과 `main` 상태가 어긋난다.
자동 배포를 도입할지, 도입한다면 어디까지를 자동화 범위로 볼지 정해야 한다.
...
선택지 C를 사용한다. `main` 푸시 시 다음 경로로 백엔드를 자동 배포한다.

```
GitHub main → CodePipeline → CodeBuild(bootJar) → S3 아티팩트
...
- 자동 배포는 `main`에 병합되면 즉시 반영된다는 뜻이다. DEC-025의 리뷰 1인 승인 규칙이 사실상 유일한 방어선이 된다.

## 영향

- 변경되는 사용자 흐름: 없음
- 변경되는 운영 흐름: DEV 백엔드 배포가 `main` 병합으로 자동 수행된다. 수동 배포 절차는 장애 대응용으로만 남는다.
