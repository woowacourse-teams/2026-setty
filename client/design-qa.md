...
- Accessibility and interaction: the mobile menu exposes `aria-expanded`/`aria-controls`, closes on Escape, and restores focus. The authentication sheet traps focus, locks background scrolling, restores prior focus, and marks the background inert. Mobile action controls meet a 44 px touch target where applicable, and reduced-motion users do not receive skeleton animation.
...
- Landscape authentication sheet: `/private/tmp/setty-responsive-auth-landscape-focus.png`
...
Primary interactions tested: mobile menu open/close and Escape, listing selection, detail back action, purchase-to-auth flow, signup/login with virtual data, authentication focus behavior, my-listings navigation, registration layout, order creation, and my-orders navigation. Browser console errors: none.
...
- `npm run build`: passed with the repository's webpack entrypoint-size advisory (260 KiB combined JS/CSS).
