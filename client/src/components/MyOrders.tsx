...
export function MyOrders() {
    ...
    return (
        <section className={cx('my-orders')} aria-labelledby="my-orders-title">
            <div className={cx('my-orders__heading')}>
                <h1 id="my-orders-title">내 주문 <span>{items.length}</span></h1>
            </div>
            {isLoading && <p className={cx('product-grid-message')}>내 주문 목록을 불러오는 중입니다.</p>}
            {error && <div className={cx('product-grid-message')}><p>{error}</p><button onClick={() => void load()} type="button">다시 시도</button></div>}
            {!isLoading && !error && items.length === 0 && <p className={cx('product-grid-message')}>주문한 가구가 없습니다.</p>}
            {!isLoading && !error && items.length > 0 && (
                <div className={cx('my-orders__list')} aria-label="내 주문 목록">
                    {items.map((item) => (
                        <article className={cx('my-orders__item')} key={item.id}>
                            <div className={cx('my-orders__details')}>
                            ...
                            <strong className={cx('my-orders__total')}>{(item.listing.price + item.listing.deliveryFee).toLocaleString('ko-KR')}원</strong>
                            <span className={cx('my-orders__status', `my-orders__status--${item.deliveryStatus.toLowerCase()}`)}>{deliveryStatusLabels[item.deliveryStatus]}</span>
                        </article>
                    ))}
                </div>
            )}
