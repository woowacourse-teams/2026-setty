...
export function MyFavorites({ onSelect }: MyFavoritesProps) {
    ...
    return (
        <section className={cx('my-orders', 'my-favorites')} aria-labelledby="my-favorites-title">
            <div className={cx('my-orders__heading')}>
                <h1 id="my-favorites-title">찜한 매물 <span>{items.length}</span></h1>
            </div>
            {isLoading && <p className={cx('product-grid-message')}>찜 목록을 불러오는 중입니다.</p>}
            {error && <div className={cx('product-grid-message')}><p>{error}</p><button onClick={() => void load()} type="button">다시 시도</button></div>}
            {!isLoading && !error && items.length === 0 && <p className={cx('product-grid-message')}>찜한 가구가 없습니다.</p>}
            {!isLoading && !error && items.length > 0 && (
                <div className={cx('my-orders__list')} aria-label="찜한 매물 목록">
                    {items.map((item) => (
                        <article className={cx('my-orders__item', 'my-favorites__item')} key={item.id}>
                            <button className={cx('my-favorites__select')} onClick={() => onSelect(item.id)} type="button">
                                <span className={cx('my-favorites__thumbnail')}>
                                    {item.thumbnailUrl && <img src={item.thumbnailUrl} alt="" />}
                                </span>
                                <span className={cx('my-orders__details')}>
                                    <h2>{item.title}</h2>
                                    <p>매물가 {item.price.toLocaleString('ko-KR')}원 <i /> 배송비 {item.deliveryFee.toLocaleString('ko-KR')}원</p>
                                </span>
                            </button>
                            <strong className={cx('my-orders__total')}>{item.totalPrice.toLocaleString('ko-KR')}원</strong>
                            <button
                                className={cx('my-favorites__remove')}
            ...
            {removeMessage && <p className={cx('product-grid-message')} role="status">{removeMessage}</p>}
        </section>
    );
}
