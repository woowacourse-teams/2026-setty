...
export function FurnitureRegistration({ listing, onCancel, onDirtyChange, onSaved }: FurnitureRegistrationProps) {
    ...
    return (
        <section aria-labelledby="furniture-registration-title" className={cx('furniture-registration')}>
            <h1 id="furniture-registration-title">{listing ? '가구 수정' : '새 가구 등록'}</h1>

            <form onSubmit={(event) => void handleSubmit(event)}>
                <fieldset className={cx('furniture-registration__field', 'furniture-registration__field--images')}>
                    <legend>사진</legend>
                    <input
                        accept="image/*"
                        aria-label="가구 사진 선택"
                        className={cx('furniture-registration__image-input')}
                        id="furniture-registration-images"
                        multiple
                        onChange={changeImages}
                        type="file"
                    />
                    <div className={cx('furniture-registration__image-list')}>
                        {listing?.images.filter((image) => retainedImageIds.includes(image.id)).map((image) => (
                            <span className={cx('furniture-registration__image-slot')} key={image.id}>
                                <img alt="기존 가구 사진" src={image.url} />
                                <button aria-label="기존 사진 삭제" className={cx('furniture-registration__image-remove')} onClick={() => removeExistingImage(image.id)} type="button">
                                    <X aria-hidden="true" weight="bold" />
                                </button>
                            </span>
                        ))}
                        {imageUrls.map((imageUrl, index) => (
                            <span className={cx('furniture-registration__image-slot')} key={imageUrl}>
                                <img alt={`선택한 가구 사진 ${index + 1}`} src={imageUrl} />
                                <button aria-label="새 사진 삭제" className={cx('furniture-registration__image-remove')} onClick={() => setImages((current) => current.filter((_, fileIndex) => fileIndex !== index))} type="button">
                                    <X aria-hidden="true" weight="bold" />
                                </button>
                            </span>
                        ))}
                        {retainedImageIds.length + images.length < 5 && (
                            <label aria-label="가구 사진 추가" className={cx('furniture-registration__image-slot', 'furniture-registration__image-slot--add')} htmlFor="furniture-registration-images">
                                <Plus aria-hidden="true" className={cx('furniture-registration__add-icon')} weight="regular" />
                            </label>
                        )}
                    </div>
                </fieldset>

                <label className={cx('furniture-registration__field')}>
                    <span>제목</span>
                    <input
                        maxLength={100}
                ...
                <fieldset className={cx('furniture-registration__field')}>
                    <legend>카테고리</legend>
                    <div aria-label="카테고리" className={cx('furniture-registration__chips')} role="radiogroup">
                        {categories.map((item) => (
                            <button
                                aria-checked={category === item.value}
                                className={cx('furniture-registration__chip', category === item.value && 'furniture-registration__chip--selected')}
                                key={item.value}
                ...
                <fieldset className={cx('furniture-registration__field')}>
                    <legend>상태</legend>
                    <div aria-label="가구 상태" className={cx('furniture-registration__chips')} role="radiogroup">
                        {conditionGrades.map((grade) => (
                            <button
                                aria-checked={conditionGrade === grade}
                                className={cx('furniture-registration__chip', conditionGrade === grade && 'furniture-registration__chip--selected')}
                                key={grade}
                ...
                <fieldset className={cx('furniture-registration__field')}>
                    <legend>크기 (cm)</legend>
                    <div className={cx('furniture-registration__dimensions')}>
                    ...
                    <p className={cx('furniture-registration__hint')}>예상 배송비 크기 입력 필요</p>
                </fieldset>

                <label className={cx('furniture-registration__field')}>
                    <span>가격</span>
                    <div className={cx('furniture-registration__price-input')}><b>₩</b><input aria-label="가격" inputMode="numeric" min="0" onChange={(event) => setPrice(event.target.value)} placeholder="180000" type="number" value={price} /></div>
                </label>

                <label className={cx('furniture-registration__field', 'furniture-registration__field--description')}>
                    <span>설명</span>
                    <textarea maxLength={1000} onChange={(event) => setDescription(event.target.value)} placeholder="사용 기간, 흠집, 분해 여부" value={description} />
                </label>

                {error && <p className={cx('furniture-registration__error')} role="alert">{error}</p>}
                <div className={cx('furniture-registration__actions')}>
                    <button className={cx('furniture-registration__cancel')} onClick={onCancel} type="button">취소</button>
                    <button className={cx('furniture-registration__submit')} disabled={!canSubmit || isSubmitting} type="submit">{isSubmitting ? '저장 중...' : listing ? '수정하기' : '등록하기'}</button>
                </div>
            </form>
        </section>
    );
