...
function formatRegisteredAt(createdAt: string) {
    ...
    return (
        <article
            className={cx('product-card', 'product-card--interactive')}
            role="button"
            tabIndex={0}
            onClick={onSelect}
            onKeyDown={(event) => {
        ...
        >
            <div className={cx('product-card__image-wrap')}>
                {product.thumbnailUrl && (
                    <img className={cx('product-card__image')} src={product.thumbnailUrl} alt={product.title} />
                )}
            </div>
            <div className={cx('product-card__details')}>
                <h2 className={cx('product-card__title')}>{product.title}</h2>
                <p className={cx('product-card__price')}>
                    {formatWon(product.price)} <span>+ 배송 {formatWon(product.deliveryFee)}</span>
                </p>
                <p className={cx('product-card__total')}>총 {formatWon(product.totalPrice)}</p>
...
function ProductCardSkeleton() {
    return (
        <article className={cx('product-card')} aria-label="매물 목록을 불러오는 중">
            <div className={cx('product-card__image-wrap', 'skeleton')} />
            <div className={cx('product-card__details')}>
                <div className={cx('skeleton', 'skeleton--title')} />
                <div className={cx('skeleton', 'skeleton--price')} />
...
export function ProductGrid({ onProductSelect, searchQuery }: ProductGridProps) {
    ...
    if (isLoading) {
        return (
            <section className={cx('product-grid')} aria-label="판매 중인 가구 목록">
    ...
    if (error) {
        return (
            <section className={cx('product-grid-message')} aria-live="polite">
                <p>{error}</p>
    ...
    if (filteredItems.length === 0) {
        if (trimmedSearchQuery) {
            return <p className={cx('product-grid-message')}>&ldquo;{trimmedSearchQuery}&rdquo;에 해당하는 매물이 없습니다.</p>;
        }

        return <p className={cx('product-grid-message')}>판매 중인 매물이 없습니다.</p>;
    }
