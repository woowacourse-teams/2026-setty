...
export function MyListings({ onRegister, onEdit }: MyListingsProps) {
    ...
    return (
        <section className={cx(accountPagesStyles['my-furniture'])} aria-labelledby="my-furniture-title">
            <div className={cx(accountPagesStyles['my-furniture__heading'])}>
                <h1 id="my-furniture-title">내 가구 <span>{items.length}</span></h1>
                <button className={cx(accountPagesStyles['my-furniture__register-button'])} onClick={onRegister} type="button">새 가구 등록</button>
            </div>
            {isLoading && <p className={cx(productGridStyles['product-grid-message'])}>내 가구 목록을 불러오는 중입니다.</p>}
            ...
            {!isLoading && !error && items.length === 0 && <p className={cx(productGridStyles['product-grid-message'])}>등록한 가구가 없습니다.</p>}
            {!isLoading && !error && items.length > 0 && (
                <div className={cx(accountPagesStyles['my-furniture__list'])} aria-label="등록한 가구 목록">
                    {items.map((item) => (
                        <article className={cx(accountPagesStyles['my-furniture__item'], item.saleStatus === 'SOLD' && accountPagesStyles['my-furniture__item--sold'])} key={item.id}>
                            <div className={cx(accountPagesStyles['my-furniture__thumbnail'])}>
                                {item.thumbnailUrl && <img alt="" src={item.thumbnailUrl} />}
                            </div>
                            <div className={cx(accountPagesStyles['my-furniture__details'])}>
                                <h2>{item.title}</h2>
                                <p>{categoryLabels[item.category]} <i /> {item.conditionGrade}급 <i /> {formatRegisteredAt(item.createdAt)} 등록</p>
                            </div>
                            <strong className={cx(accountPagesStyles['my-furniture__price'])}>{item.price.toLocaleString('ko-KR')}원</strong>
                            <span className={cx(accountPagesStyles['my-furniture__status'], item.saleStatus === 'RESERVED' && accountPagesStyles['my-furniture__status--reserved'])}>{statusLabel(item.saleStatus)}</span>
                            <div className={cx(accountPagesStyles['my-furniture__controls'])}>
