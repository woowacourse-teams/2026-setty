...
/** 판매자 화면처럼 값을 바꿀 수 없는 곳에서 쓰는 읽기 전용 표시 */
export function HighValueValue({ highValueItem }: { highValueItem: boolean }) {
  return (
    <div className={styles.field}>
      <span className={styles.label}>{LABEL}</span>
      <p className={styles.readOnly}>{highValueItem ? '50만원 이상' : '50만원 미만'}</p>
    </div>
  );
}
